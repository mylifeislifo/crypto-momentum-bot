...
| 항목 | 값 | 출처 | 원본 System 1 비교 |
|------|-----|------|---|
| 진입 채널 (HHLL) | 20일 | 영상 | 동일 ✓ |
| 청산 채널 (HHLL) | 11일 | 영상 | 원본 10일 (1일 차이, 영상 변형) |
| 손절 | ATR(20) × 2 | 영상 | 동일 ✓ |
| 추세 필터 | 200일 SMA | 영상 | **원본 없음** — 영상 추가 기능 |
| 리스크 / 트레이드 | 2% | 영상 | 원본 1유닛 = 계좌 2% 리스크 ✓ |
| 진입 방향 | 200SMA 위 = 롱 중심 / 아래 = 숏 중심 | 영상 | **원본 없음** — 영상 추가 |
| 피라미딩 | 미구현 | M1 단순화 | 원본 최대 4유닛(+0.5ATR마다 추가) |

> **200SMA 필터 해석 주의**: BTC/ETH가 2019년 이후 장기 우상향이므로, 이 필터가 추세 추종 알파 외에 "BTC 장기 상승 베타"도 일부 포착할 수 있음. M2 연도별 감쇄 분석이 필수인 이유.

> **원본 System 1 스킵 룰 미적용**: 원본은 "직전 System 1 신호가 수익이었으면 다음 동일 방향 신호 건너뜀 (55일 돌파에서 안전장치 진입)"을 포함한다. 미적용 시 원본 대비 진입 횟수가 더 많을 수 있음.

거래 비용 가정: taker **0.06%** × 2(in/out) + 슬리피지 0.05% = 왕복 **0.17%**. (Bitget USDT-M VIP 0 실제 taker 0.06% 기준 — 구 0.04%는 오류였음, `config.py` 수정 완료)
...
체결, 매 트레이드 **거래비용 차감**(taker **0.06%**×2 + 슬리피지 0.05% = 왕복 0.17%). 봉당 1포지션,
심볼별 독립 자본(50/50).

**M1 낙관적 편향 목록 (결과 해석 시 차감 고려)**:
- 손절 갭 없음 가정: stop 가격 그대로 체결. BTC 5~10% 갭 발생 시 실제 손실 더 큼.
- 펀딩 비율 미반영: 무기한 선물 8시간마다 정산. 30일 보유 × 0.03%/일 = 약 **+0.9% 추가 원가** (보수적), 강세장 극단 시 +3%까지 가능. 롱 편향 전략이므로 실제 성과는 백테스트 대비 낮을 것.
