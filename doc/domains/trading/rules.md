...
- **[2026.06.13] (비트겟 거래소 게이트웨이 신설 — winner-asymmetry 봇을 Bitget에서 가동, 게이트 미통과 / 실거래 전 검증 필요)** 사용자 방향: 비트겟 마켓플레이스의 마티게일(스크린샷)이 아니라 **우리 winner-asymmetry 청산 규율(L3: breakeven·시간스톱·ATR 트레일)을 비트겟에서 돌린다.** winner-asymmetry는 이미 `order_manager`+`risk/trail.py`에 있고 **`FuturesGateway` ABC 뒤에 거래소-비종속**이므로, 적용 = **`BitgetFuturesGateway` 신규 구현 + 배선**(L3 청산 로직 재구현 0줄, 그대로 재사용). 구현: `src/bot/execution/bitget_futures.py` — Bitget v2 Mix(USDT-M 무기한) REST, HMAC-SHA256 서명(ACCESS-KEY/SIGN/TIMESTAMP/PASSPHRASE), one-way(net) 포지션 모드 + `reduceOnly`로 SL/청산, STOP_MARKET → **plan-order 라우팅**(place/cancel-plan-order, plan id 추적), 전 구간 `Decimal`(§1.2), 레버리지 2x캡 assert(§1.1), tenacity 재시도(§4). 배선: `enums.Exchange`(binance/bitget) + `ExchangeCfg.name/product_type/margin_coin`, `Secrets.bitget_*`(.env, 평문 금지 `security §1.1`), `FuturesGateway`에 no-op `connect/disconnect` 기본 추가 후 main이 무조건 호출(거래소 N개 일반화), `_build_gateway`가 live+bitget 시 선택. CLI `btcbot bitget`(`config/bitget.yaml` override). **현재 `config/bitget.yaml` mode: paper → 비트겟 게이트웨이는 live 플립 시에만 활성**(paper는 `PaperFuturesGateway`가 시뮬). **게이트웨이는 라이브 미검증** — Bitget demo(productType `SUSDT-FUTURES`)로 `§1.3` 게이트(backtest→walkforward→paper 7일→시드 10%)·`progressive-gate §2.4` 사용자 승인 통과 전 실거래 금지. 시크릿 평문 미포함(`.env.example`만). 테스트: bitget 게이트웨이 mocked-HTTP 16종 + config 4종 = 20종(전체 211 통과).
