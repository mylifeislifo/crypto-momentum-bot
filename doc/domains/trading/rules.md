...
- **[2026.06.12 v2] (라이브 L3 시간스톱 구현 — winner-asymmetry "손실 짧게" 완성, paper 단계 / 신규 알파 아님)** 직전 [2026.06.12]의 breakeven("이익 길게")에 이어 그 짝인 시간스톱("손실 짧게")을 구현 → winner-asymmetry(1491: 손실 47h vs 이익 178h 비대칭)를 코드로 완성. §3 표의 "시간 타이트닝 도입됨"도 실제 라이브 코드엔 없던 드리프트였음 → 해소. **메커니즘**: `risk/trail.py`가 5m봉당 `bars_held`를 세고 `due_time_exits()`가 (a) **미검증 포지션**(breakeven 미무장 = +trigger% 한 번도 못 도달) `time_stop_bars` 경과 시 시장가 청산, (b) `max_hold_bars` 하드캡(0=비활성). **검증된 winner(be_armed)는 시간스톱 면제** → "이익 길게"와 직교(트레일로 free run). `order_manager._force_close`(paper: `close_position` / live: reduce-only MARKET + SL 취소)로 새 청산 경로 추가, per-5m봉 훅에서 호출. config `time_stop_bars=48`(4h, **초기값 — paper 백테스트로 튜닝 필요**)·`max_hold_bars=0`. 검증자: 음수 거부 + `max_hold ≥ time_stop`(하드캡이 조건부 컷 아래면 거부). 정보적 알파 아닌 **행동적 청산 규율**(`signal-validation §3` [2026.06.11] 구분). **현재 `mode: paper`/`dry_run: true` → §1.3 게이트 전 실거래 영향 없음.** `core/types.py`에 `ForcedExit` 신설. 테스트: test_trail 시간스톱 6종 + test_risk_config 시간스톱 4종(전체 173 통과).
- **[2026.06.12 v3] (재시작 생존 — winner-asymmetry 실효화, paper 단계)** L3 점검 중 발견: `main.py`가 graceful 종료 시 `close_all_positions`로 **전량 flatten** → bot-ops §2.1 SIGTERM 재시작(주 3회)마다 **검증 winner까지 청산** → #13·#14("이익 길게")가 재시작 주기에 무력화. 더해 트레일 상태(peak·be_armed·bars_held)가 in-memory라 크래시 시 소실(특히 `be_armed` 리셋 시 검증 winner가 재시작 후 시간스톱에 잘림). **수정 3종**: (1) config `risk.flatten_on_shutdown=false`(기본) — graceful 종료 시 **hold**(true면 구 동작). (2) `risk/trail.py` 상태 JSON 영속화(`trail_state.json`, register/on_new_bar/on_close마다 저장, 생성 시 load). (3) `order_manager.recover_positions()`(run 시작 시 1회) — 트레일↔게이트웨이 포지션 정합(고아 트레일 드롭 / 누락분 degraded 재등록) + `guard.recover_open_count()`로 open count 복구(fresh guard가 0으로 오판해 max_positions 초과 진입하는 것 차단). 다운타임은 **서버사이드 SL이 보호**(live). 이는 실제 프로덕션 봇(서버 스톱 + 재시작 복구)의 현실적 동작 — paper도 이를 충실히 시뮬. **현재 `mode: paper`/`dry_run: true` → §1.3 게이트 전 실거래 영향 없음.** 테스트: 트레일 영속화 3종 + 복구 통합 2종 + flatten config 1종(전체 183 통과).
- **[2026.06.12 v4] (F5 — 라이브 SL 무방비 버그 수정, live 전용)** L3 점검의 마지막 노트 F5를 닫던 중, "잠깐의 SL 공백"이 아니라 **영구 제거**임을 확인: `order_manager._amend_sl`이 live에서 `qty`를 `Decimal("0")`로 고정(paper일 때만 설정)해 `if qty>0`이 거짓 → **새 SL을 안 걸고 기존 SL만 취소** → 첫 트레일 이동 후 **라이브 포지션이 SL 없이 방치**. **수정**: live 경로를 **place-before-cancel**로 재작성 + `get_position`으로 **실제 포지션 qty 조회**해 새 STOP_MARKET 배치 → 성공 후에만 old 취소. placement 실패 시 **old SL 유지**(취소 안 함) → 포지션 항상 보호. paper는 원자적 `update_sl_price`로 분리(주문 공백 없음). **현재 `mode: paper`라 운영 영향 0이나, live 전환 시 첫 트레일 이동에서 스톱이 사라졌을 치명 버그를 사전 제거.** 테스트: 가짜 live 게이트웨이 2종(place-before-cancel 순서 / placement 실패 시 old 유지). 전체 185 통과. 이로써 L3 감사 노트(F1~F5) 전부 해소.
- **[2026.06.12 v5] (L3 청산 거동 검증 — winner-asymmetry 창발 확인 + whipsaw 발견, paper 단계)** 단위테스트(부분)만으론 못 보는 **창발 거동**을 검증하려 하네스 신설: `research/l3_exit_sim.py`(+`scripts/sim_l3_exits.py`)가 **실제 `TrailingStopManager`**(ATR트레일+BE+시간스톱)를 합성 5m 경로에 직접 태워 청산 결과(사유·보유봉·손익)를 정량 측정. **맥미니 불요** — `§1.3` backtest 단계의 exit-discipline 검증. **확인된 winner-asymmetry**: runaway winner 200봉/+49% 완주(open) · 미검증 grind 48봉 `time_stop`(+0.5%, 자본 회수) · 크래시 1봉 `stop_loss`(−0.65%) · **+1.6% 후 반전한 winner도 −1.8%가 아니라 +0.85%로 보호**(round-trip 불가) · SHORT 미러 동작. **🔍 발견(코드 변경 없이 기록만)**: ATR×1.5 트레일이 진입 즉시 `peak=entry`에서 조이기 시작 → **검증 전(pre-proof) 포지션을 whipsaw**로 조기 컷(`whipsaw_chop` 6봉 −0.22%, 달릴 수 있던 포지션 손실). 트레일 멀티플라이어 완화 또는 **검증 전 트레일 보류**가 튜닝 후보 — paper 백테스트로 결정. 행동 검증(`signal-validation §3` 정보적 알파 vs 행동적 구조 구분). **`mode: paper` → 실거래 영향 없음.** 테스트: l3_exit_sim 6종(전체 191 통과).
- **[2026.06.13] (L3 청산 파라미터 튜닝 하네스 — 실데이터 멀티-진입 백테스트)** 라이브 confluence 3중게이트가 드물게 발화 → paper 청산 표본 희소 → 청산 파라미터(time_stop·트레일·BE) 튜닝 곤란. 해결: `l3_exit_sim.backtest_exits()` + `scripts/backtest_l3_exits.py` — 실제 5m CSV(또는 합성)에 `entry_stride`로 진입을 샘플(청산은 entry-agnostic, R5)해 **실제 `TrailingStopManager`**를 각 진입에 적용, exit-discipline 통계(win·E[r]·**asymmetry**(winner보유/loser보유)·**whipsaw**·proven·청산사유 히스토그램) 산출 + `time_stop_bars × trail_atr_multiplier` 스윕. **합성 random-walk 검증 발견**: whipsaw는 **트레일 멀티에만 의존**(×1.5≈62%, ×3.0≈36%)·time_stop 무관, asymmetry ~2.7-3.0 구조적, 엣지 없는 데이터선 트레일이 청산을 지배(시간스톱 거의 안 묶임) → #17 whipsaw 발견을 정량 확증, 트레일 멀티가 핵심 레버임을 시사(단 실데이터로 확정). 자기버그 정정: whipsaw 윈도우를 `time_stop//4`(설정 의존, 스윕 오염) → **고정 6봉**으로. **맥미니 사용**: 과거 BTC 5m CSV를 `--csv`로 넣으면 실데이터 대량 튜닝(오프라인 분석, `mode:paper` 무관, §1.3 게이트 전 실거래 영향 0). 테스트: backtest 3종(전체 194 통과).
- **[2026.06.14] (외부 수동 그리드 봇 파라미터 감사 — §1.1 레버리지 하드캡 위반 식별, 외부 거래소·레포 코드 무관)** 사용자 운영 외부 거래소 선물 그리드 봇 6종(WIF·DOGE·PEPE·SOL·XRP·BTC) 파라미터 스크린샷 분석. 현재가는 6종 모두 그리드 레인지 내(웹 시세 기준 근사 — 즉시 이탈 없음, 시장 소폭 상승으로 롱 4종 추세 정합·XRP 숏 유리)이나 다음 리스크 결함 식별: **(1) 6종 전부 §1.1 레버리지 2x 하드캡 위반** — 3x(WIF·PEPE)·5x(DOGE·SOL)·**20x(XRP·BTC, 청산버퍼 ~5%)**. 본 봇들은 외부 거래소 수동 운영이라 코드 레벨 차단 미적용이나, 도메인 룰상 우회 불가 원칙 위반이므로 최우선 시정 대상. **(2) SOL 5x 롱 SL(54.5)이 청산추정가(~55.2) 아래** → SL 발동 전 청산, 보호 무력. SL을 청산가 위로 상향 필요. **(3) DOGE Neutral 5x SL 전무** → 한 방향 추세 시 손실측 무방비. **(4) BTC 20x SL(61,495)이 청산추정(~61,195)에 ~0.5% 근접** + 현재가가 SL까지 ~2.6%만 남음 → 모니터링. **(5) reserve funds 6종 중 5종 "No"**(XRP만 Yes) → 고레버와 결합 시 위험 증폭. 양호: WIF·PEPE(3x)는 SL이 청산가 위로 견실(PEPE TP/SL 양쪽). 청산가 수치는 격리/교차·유지증거금 티어·그리드 인벤토리 의존 **근사치** → 거래소 표기 청산가로 대조 필수(`skills/bot-ops.md §2.2` 외부값 신뢰성 0 적용 — 본 분석은 실시간 거래소 API 직접검증 미수행, 웹 시세·스크린샷 기반). 본 봇들은 레포 모멘텀 봇(paper)과 별개 수동 운영으로 §1.3 게이트 비적용 대상이나, 레버리지 2x 원칙은 사용자 리스크 기준으로 동일 적용 권장.
- **[2026.06.14 v2] (외부 그리드 봇 6종 파라미터 최적화 — 근사 데이터 기반, 백테스트 미완)** 직전 [2026.06.14] 감사 후속으로 가격 히스토리 기반 최적화 요청. **정밀 OHLCV 수신 시도했으나 이 세션 egress allowlist/DNS로 모든 klines 호스트(binance·data.binance.vision·coingecko·coincap·yahoo·kraken·bybit·bitget 등) 차단** + WebFetch는 도달하나 403(봇차단). 사용자가 allowlist 추가했으나 실행 중 컨테이너엔 미반영(egress 정책은 세션 시작 시 고정 → 새 세션 필요). → **WebSearch 근사치(일 단위 레인지·지지/저항·변동성·F&G)만으로 원칙 기반 최적화 수행. 정밀 백테스트 아님** (`skills/signal-validation.md` 신뢰성 0 적용 — 거래소 실측 OHLCV로 대조 필수). **시장 맥락(검증의 핵심)**: BTC ATH(126,200) 대비 −51%(~63~64k), 일일 청산 $1.8B, F&G **12(2026 최저)** = 자본항복·극단공포. 6종 중 5종이 롱/중립으로 **하락추세 역행** — 그리드는 횡보장 최적이라 regime 부적합, **XRP 숏만 추세 정합**(2026 저점 1.05 터치). **권장 최적값(레버리지 전부 2x 강제)**: BTC 롱 레인지 60,000–65,500/SL 59,000(지지 61,486 아래로 매수여력 확보), SOL 롱 58–78/SL 55(2x화로 기존 SL<청산 버그 자동 해소), XRP 숏 1.06–1.20/SL **1.22**(천장 1.20 위로 — 기존 1.19는 천장 아래 모순), DOGE 중립 0.078–0.098 + **양방 비상 exit 0.074·0.101**(기존 SL 전무 해소), PEPE 롱 2.0e-6–3.3e-6/SL 1.9e-6/TP 3.6e-6(이미 최선 설정, 레버만 하향), WIF 롱 0.13–0.19(상단 0.23 유휴 트림, 현재 ~0.16 하락추세)/SL 0.12. **공통**: reserve funds 전부 활성, 하락국면이므로 롱 노출 축소·XRP 숏 유지 우선. 그리드 간격은 6종 모두 왕복수수료(~0.1%) 충분 상회로 적정. **실측 기반 정밀 최적화는 다음 세션(allowlist 반영) 또는 거래소 CSV(ts,open,high,low,close) 제공 시 레포 백테스트 툴로 수행 예정.** §1.3 게이트·2x 하드캡 원칙 동일 적용.
