...
- **[2026.06.13] (비트겟 Spot Martingale(DCA) 봇 신설 — M1 백테스트 스켈레톤, 게이트 미통과 / 실거래 금지)** 사용자 요청으로 비트겟 "Spot Martingale" 전략봇(BGB/USDT·Aggressive 스크린샷)을 별도 패키지 `src/martingale_bot/`(config·grid·engine)로 복제. `turtle_bot`과 동일하게 `src/bot/` 라이브 봇과 코드 완전 별도. 스크린샷 파라미터 박음: drop 1% · TP 1% · max SO 5 · vol_scale 2.5 · step_scale 1.0 · 즉시진입. **하드룰 적용**: 전 구간 `Decimal`(§1.2) · 레버리지 현물 1x·2x캡 validator(§1.1) · JSON Lines 거래로그 `{symbol,side,qty,price,pnl}`(§5 + `audit-log §2.1`). **🔴 노하우 충돌 명시**: 마티게일은 본 레포 핵심 발견 winner-asymmetry(R5 "손실 짧게·이익 길게")의 **정반대** — 손실에 물타기(let losers run)·이익 +1% 컷(cut winners short). 잦은 소액 익절로 겉보기 APY는 양수지만(스크린샷 +0.85%) **fat-tail 1건이 누적을 통째로 날림**(C-2 [2026.06.11] "2건 손실에 좌우"와 동형). M1 합성 백테스트가 이를 정량 노출: grind_up/chop_range는 +0.1~0.3%인데 **crash 단일 레짐 −32%·maxDD 32%·stuck bag**. 풀래더 자본요구 = base×65배(`max_cycle_cost`)로 자본함정도 노출. **선택적 winner-asymmetry 오버레이** `hard_stop_pct`(기본 0=바닐라 비트겟, >0이면 사이클 손절)로 레포 노하우 적용 지점 제공. 평가는 평균·APY 아닌 **fat-tail 크기·빈도**로(`signal-validation §2.1·§2.2`). **M1 = backtest 점추정만 → §1.3 게이트(backtest→walkforward→paper 7일→시드 10%)·`progressive-gate §2.4` 사용자 승인 전 실거래 금지.** 라이브 비트겟 API 연동·시크릿 미포함(의도적). 테스트: config 28 + grid 16 + engine 18 = 62종(전체 253 통과).
